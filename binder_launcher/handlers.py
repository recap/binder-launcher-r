...
#                Values: "1", "0" (default)
#
# install_r_deps: Whether to install R dependencies after staging.
#                Values: "1" (default), "1"
#
...
def install_r_dependencies(target: Path, log):
    rscript = shutil.which("Rscript")

    if rscript is None:
        log.info("Rscript not found; skipping R dependency installation")
        return

    candidates = [
        target / "binder" / "install.R",
        target / ".binder" / "install.R",
        target / "install.R",
        target / "DESCRIPTION",
    ]
    ...
    runtime_candidates = [
        target / "runtime.txt",
        target / "binder" / "runtime.txt",
        target / ".binder" / "runtime.txt",
    ]

    for runtime in runtime_candidates:
        if runtime.exists():
            log.info("Found R runtime file: %s", runtime)
            log.info("runtime.txt contents:\n%s", runtime.read_text())

    for path in candidates:
        if not path.exists():
            continue

        log.info("Found R dependency file: %s", path)

        if path.name == "install.R":
            ...
            cmd = [
                ...
                (
                    ...
                    f"remotes::install_deps('{target}', dependencies=TRUE, upgrade='never')"
                ),
            ]
        ...
        log.info("R install stdout:\n%s", result.stdout)
        log.info("R install stderr:\n%s", result.stderr)

        if result.returncode != 0:
            raise RuntimeError(
                f"R dependency installation failed:\n{result.stderr}"
            )

        return

    log.info("No R dependency files found")
...
class LaunchHandler(JupyterHandler):
    ...
    def get(self):
        ...
        install_r_deps = self.get_argument("install_r_deps", "1") == "1"
        ...
        try:
            ...
            if install_r_deps:
                install_r_dependencies(target, self.serverapp.log)
            if run_postbuild:
